...
__gds-task-view_dynamic_comp() {
    ...
    _gds-task-view_compreply ${comp[@]}
}

function __gds-task-view_handle_options() {
    local i j
    declare -a copy
...
function __gds-task-view_handle_flags() {
    local i j
    declare -a copy
    local last="${MYWORDS[$INDEX]}"
    local max=`expr ${#MYWORDS[@]} - 1`
...
__gds-task-view_handle_options_flags() {
    __gds-task-view_handle_options
    __gds-task-view_handle_flags
}
...
__comp_current_options() {
    ...
    if [[ -n $always || ${MYWORDS[$INDEX]} =~ ^- ]]; then
      ...
      __gds-task-view_dynamic_comp 'options' "$options_spec"
...
complete -o default -F _gds-task-view gds-task-view
