...
__gds-view_dynamic_comp() {
    ...
    _gds-view_compreply ${comp[@]}
}

function __gds-view_handle_options() {
    local i j
    declare -a copy
...
function __gds-view_handle_flags() {
    local i j
    declare -a copy
    local last="${MYWORDS[$INDEX]}"
    local max=`expr ${#MYWORDS[@]} - 1`
...
__gds-view_handle_options_flags() {
    __gds-view_handle_options
    __gds-view_handle_flags
}
...
__comp_current_options() {
    ...
    if [[ -n $always || ${MYWORDS[$INDEX]} =~ ^- ]]; then
      ...
      __gds-view_dynamic_comp 'options' "$options_spec"
...
complete -o default -F _gds-view gds-view
