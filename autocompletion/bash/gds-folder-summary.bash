...
')
    __gds-folder-summary_handle_options_flags
...
gds-ls "${CURRENT_WORD}" 2>/dev/null
    ...
    _gds-folder-summary_compreply "$param_gds_path"
}
...
__gds-folder-summary_dynamic_comp() {
    ...
    _gds-folder-summary_compreply ${comp[@]}
}

function __gds-folder-summary_handle_options() {
    local i j
    declare -a copy
...
function __gds-folder-summary_handle_flags() {
    local i j
    declare -a copy
    local last="${MYWORDS[$INDEX]}"
    local max=`expr ${#MYWORDS[@]} - 1`
...
__gds-folder-summary_handle_options_flags() {
    __gds-folder-summary_handle_options
    __gds-folder-summary_handle_flags
}
...
__comp_current_options() {
    ...
    if [[ -n $always || ${MYWORDS[$INDEX]} =~ ^- ]]; then
      ...
      __gds-folder-summary_dynamic_comp 'options' "$options_spec"
...
complete -o default -F _gds-folder-summary gds-folder-summary
